...
class TestHook(TestCase):
    ...
    def test_reads_content_to_hook(self, mock_read_lines, _):
    ...
    def test_is_guet_hook_returns_whether_hook_matches_guet_content(self,
                                                                    mock_read_lines,
                                                                    _):
    ...
    def test_works_with_specific_content(self, mock_read_lines, _):
    ...
    def test_init_with_create_flag_saves_content_to_default(self,
                                                            mock_read_lines,
                                                            _):
    ...
    def test_init_with_create_flag_overwrites_already_present_content(self, mock_read_lines, _):
        mock_read_lines.return_value = ['Other', 'Content']
        hook = Hook(Path('/path/to/.git/hooks/name'), create=True)
        self.assertEqual(PYTHON3_GUET_HOOK, hook.content)
    ...
    def test_save_writes_lines_to_file(self, mock_write_lines, _1, _2, mock_read_lines, _3):
        mock_read_lines.side_effect = FileNotFoundError()
        mock_path = Mock()
    ...
    @patch('guet.git.hook.stat')
    @patch('guet.git.hook.write_lines')
    def test_save_chmods_file_to_executable(self, _1, mock_stat, mock_chmod, mock_read_lines, _2):
        mock_read_lines.side_effect = FileNotFoundError()
    ...
    def test_is_get_hook_registers_python_shebang_without_the_3_as_guet_hook(self,
                                                                             mock_read_lines,
                                                                             _):
    ...
    @patch('guet.git.hook.stat')
    @patch('guet.git.hook.write_lines')
    def test_uses_python_if_python3_is_not_available(self,
                                                     mock_write_lines,
                                                     _1,
                                                     _2,
                                                     mock_read_lines,
